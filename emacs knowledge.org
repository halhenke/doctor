...
** Eval and store/copy the outpit of a non-interactive command
*** With buffer and prefix-argument
If you do a prefix arg before 
: eval-expression
such as 
: C-u M-:
then it will insert result into current buffer at point.
*** With 'values'
The result of eval-expression is "consed to the front of the variable values
Suppose you could eval this in scratch or do
: (describe-variable 'values)
and copy from that buffer
...
* Making Emacs Faster/Snappier
** Delay Fontification
From http://tsengf.blogspot.com.au/2012/11/slow-scrolling-speed-in-emacs.html
#+BEGIN_VERSE
...
not
: \\(hulk\\|crazy\\)
*** Trying to grep for "negative expressons" - i.e. looking for *commit messages* that DONT contain a given regexp
See this [[file:Git%20Usage%20Notes.org::*Show%20all%20commits%20whose%20message%20does%20NOT%20contain%20a%20certain%20regexp][git log: Showing all commits whose message does NOT contain a certain regexp]]
** Troubleshooting
...
** Using for multiple repositories
 - Set magit-repo-dirs to a list of directories
 - Following command to switch between multiple repos
...
: ediff-current-file
* find-grep
** Proper syntax for a reg-exp type search
This works - no need to do emacs style escapes it seems - it prob runs a command line version
: find ~/bridge/app/models/ -type f -exec grep -nH -e "validate.*email" {} +
* isearch
** isearch variables
: isearch-string
...
*** Can call  on more than one directory
: ctags -eR app lib
...
** Cleaning up after a badly installed package
If its an elpa package there will be symlinks into the el-get package directory. These will be removed.

May have to change two files
: ~/.emacs.d/el-get/.loaddefs.el
and
: ~/.emacs.d/el-get/.status.el
set the status to something like
: (sublime status "removed" recipe nil)
remove the definitions from loaddefs
which will begin with something like
#+BEGIN_VERSE
...
#+END_VERSE
and then remove the mention at the end of the file if necessary:
: ;;;;;;  "slime/slime-autoloads.el" "slime/slime.el"
* elpa
...
*** DONE KEYBINDINGS - Good available keybindings:
-  <s-Return>
...
*** TODO Some form of command binding to help - (global-set-key (remap 'x) 'y) ? ;x and y have to be functions not key bindings in that case
*** TODO Bind Set mark - <Command Spacebar> ? (Prob not as necessary now that shift select is working good)
...
***** TODO Rspec
***** TODO FactoryGirl
**** DONE Better Default Bindings
**** TODO Set outline-reg-exp for different modes/files extensions e.g. rake
Perhaps can be done with "make-local-variable"? without coming up with a whole new rake mode?
: (set (make-local-variable 'indent-line-function) 'haml-indent-line)

Or look at this from the "elhome" package
#+BEGIN_VERSE
...
: dired-mode
becomes active...
*** TODO Replace "Save file y,n,!,.,q,C--r,d,C-h" with an option to revert buffer
And perhaps more sensible text
**** Initial thoughts
From here
...
The actual prompt itself /may/ be customizable ia the
: confirm-kill-emacs
which should be non-nil
**** Secondary
Perhspas more related to command
: save-some-buffers
** Maybe less interested in doing now
...
** Tried to fix but not satisfactory
...
***** Custom vs Default
  ...
  - Default or quick shutdown
    - save everything
* TODO - Bigger Ideas
** TODO From emacs in ruby-mode command to run "source_location" method on a class
(Not sure if this would need a rails console running?)

Create  a command would take a class and a method
: Class.instance_method(:method).source_location
and return first result as a file and second as a line....

Perhaps visit that file/line also....
...
** TODO Create a library famework for creating new emacs UIs
There is a lot of emacs code out there - not much seems to be aimed at building blocks to help others make stuff......
*** Something object oriented?
** TODO Create a library famework for adding modern data structures etc to emacs and possibly a front end to older stuff
*** Like alists etc
*** Some syntax more like json?
*** A prototype object syntax?
*** Something object oriented?
** TODO El-get list of packages synchronisation 
Basically a command to automatically write the current list of el-get pacakges to a file that we can then use to sync with on other machines. Might reuire a new hidden settings file
** TODO Package with tabs for emacs?
** TODO A Package to help people create and publish emacs packages to elpa, el-get etc
Particularly will help to install stuff for info-mode with an easy interface
** TODO Before Init-Hook - auto-recompile
Check if files are out of date and re-byte-compile them?
* TODO - Fork/Improve Emacs Packages etc
** TODO Haml Mode
*** Comments working
Add 
: comment-start-skip
and
: comment-end-skip
*** Replace default javascript mode
Currently highlights using crappy javascript mode
Should at least provide a config value that offers to use js2-mode or perhaps even require it
** TODO el-get
*** TODO el-get-intalled function
Echo list of installed packages
*** TODO el-get-installed functions automatically sync across machines
use 
: el-get-list-installed-packages
to store package list in the buffer (???) or a special settings file
** TODO Skewer-Mode
*** DONE Full browser load solution
Basically i want something like a Chrom plugin if necessary or something i can do from emacs that will do the equivalent of running all of this from the console:
...
We need a server/ajax request to get local files but 
perhaps we can use emacs to eval a buffer/file - maybe in a skewer mode hook
** TODO Magit
*** TODO Run Magit in a separate Emacs process with only a few buffers for speed?
*** TODO Startup Magit
...
**** DONE Run revbuffs after a Pull
This actually seems to happen most of the time
**** TODO NOT run revbufs after a pull
**** TODO Push to multiple remotes
*** TODO Magit-Switch-Repo-Shortcut-macro
: C-u M-x magit-status
to something like 
: magit-choose-repo
**** TODO "My Snippets" repo pretty consistently doesnt work
How comes?
* Bugs
** TODO General
...
** Installing Packages via el-get/elpa
*** TODO Sublime Mode
Installed via el-get I get:
#+BEGIN_VERSE
Warning (emacs): Unable to activate package `sass-mode'.
Required package `haml-mode-3.0.14' is unavailable
#+END_VERSE
I hacked around this by editing
: ~/.emacs.d/el-get/package/elpa/sass-mode-3.0.14/sass-mode-pkg.el
and changing
: (define-package "sass-mode" "3.0.14" "Major mode for editing Sass files" (quote ((haml-mode "3.0.14"))))
to
: (define-package "sass-mode" "3.0.14" "Major mode for editing Sass files" (quote ((haml-mode "3.1.0"))))
**** yasnippet
load yansippet problem
**** ido-ubiquitous problem
*** 
** TODO Based on my code/changes
*** TODO I have totally broken my incremental find command by using find at point
I can see it not work in haml-mode, org-mode etc
If I evaluate 
: isearch-string 
in org-mode when looking for "--theirs" i get:
: #("Git--theirs" 0 3 (fontified t face org-level-1))
or when i did the search when the cursor was over the word "merge" i get
: #("mergetheirs" 0 5 (fontified t org-no-flyspell t face org-code))
when i go back to an elisp file and retype theirs it goes back to normal....
and similarly when i go back to an org-mode file again and do 
: isearch-repeat-forward
it works ok

So obviously there is some extra-textual information in org-mode files that confuses this feature...
*** TODO My Alt Select Code doesnt work if you go upwards 
Can choose it to work up or down
** TODO Unix
...
** TODO In Third Party Code
*** TODO igrep
**** DONE xargs -e error on Mac
On Mac I get the error
: xargs: illegal option -- e
total output:
#+BEGIN_VERSE
find /Users/Hal/.emacs.d/init-files -type d \( -name RCS -o -name CVS -o -name SCCS \) -prune -o -type f \! -name \*\~ \! -name \*\,v \! -name s.\* \! -name .\#\*  -print0 | xargs -0 -e grep -n  -e isearch  /dev/null
.zshenv called...
xargs: illegal option -- e
usage: xargs [-0opt] [-E eofstr] [-I replstr [-R replacements]] [-J replstr]
             ...
             [utility [argument ...]]
#+END_VERSE
this works on linux however
***** An explanation -
From man page on Linux
#+BEGIN_VERSE
-e[eof-str]
This option is a synonym for the -E option.  Use -E instead, because it is POSIX compliant while this option is not.  If eof-str is omitted, there is  no  end
of file string.  If neither -E nor -e is used, no end of file string is used.
#+END_VERSE
***** Solutions?
****** Redefine the command in el-get recipe after?
****** Change xargs?
****** *SOLUTION* Custom variables from igrep
: igrep-find-use-xargs
is currently set to gnu 

Set to nil to use 
: find exec
instead of xargs
****** Bulk edit Code
If i change '-e' option to '-E' then i get this error:
#+BEGIN_VERSE
xargs -0 -E grep -n  -e \(kbd\ \"s-f\"\)  /dev/null
.zshenv called...
xargs: illegal argument count
#+END_VERSE
**** igrep sometimes doesnt find things that find-grep will happily find....
igrep command
#+BEGIN_SRC shell
find /Users/Hal/.emacs.d -type d \( -name RCS -o -name CVS -o -name SCCS \) -prune -o -type f \! -name \*\~ \! -name \*\,v \! -name s.\* \! -name .\#\* -name \*.org -exec grep -n -i -e Hulk \{\} /dev/null \;
#+END_SRC
find-grep
#+BEGIN_SRC shell
find ~/.emacs.d -type f -exec grep -nH -e Hulk {} +
#+END_SRC
***** Why?
****** Quotation marks?
****** igrep -i option
