...
** Showing the complete git history of a file
: git log --follow -p path_file
To be able to properly browse inspect you prob want to do something like this if grep isnt enough:
: git log --follow -p app/jobs/jsci_create_job.rb > jsci_history
