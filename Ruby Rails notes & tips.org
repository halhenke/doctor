#+TITLE: Ruby, Rails &Related Tools - Tips, Tricks & Notes
...
** Using __FILE__ etc to load code
*** Basics
: __FILE__
is a variable that shows the path of the current file /relative from the execution directory
Therefore its a relative path. 
*** To get an absolute path from it
: File.expand_path(FILE)
*** To execute/load/inspect files relative to the current file 
the following should be sufficient
: "#{File.dirname(__FILE__)}/../hals_output.rb"
*** May work differently in Ruby 1.9.2 vs Ruby 1.9.3
To work around this then perhaps the following is preferable
...
* Using Ruby in conjunction with shell scripts - limitations
See [[file:Shell%20Scripting%20Magic.org::*Using%20a%20script%20(shell/ruby),%20subshells%20and%20environemnt%20variables][Using a script (shell/ruby), subshells and environemnt variables]] 
...
or 
: rake -D
** Calling Rake in a directory with a Rakefile
...
: rvm use default
** You can manipulate rvm from within Ruby itself by loading some code
something like this:
#+BEGIN_SRC ruby
...
#+END_SRC
*Actually* the rvm gem is included by default with rvm so just
: require "rvm"
is required i think
...
** let can be anything in a block
e.g. a series of commands to be repeated e.g.
#+BEGIN_SRC ruby
let!(:bdod) { Bdod.delete_all; Bdod.create!(:user => create(:user), :report_name => "rt2691_ebiw_jsc01_ext1_F790", :site => create(:site, :organisation => organisation), :organisation_id => organisation.id)}
#+END_SRC
...
** Stubbing
This is some hash syntax
: @form.stub!(:push_as_ess_comment? => false , :show_push_comment? => false)
** Rspec Rails
...
** Using asociations with traits and factories that have different names than the association
requires a special kind of syntax
#+BEGIN_SRC ruby
factory :rails_process_jrrr_job, class: ProcessJrrrJob do        
        queue { "rails" }
        association :refer_job, :factory => :windows_process_jrrr_job
        trait :requested_date do           
          association :refer_job, :factory => [:windows_process_jrrr_job, :requested_date ]
        end
      end
#+END_SRC
** Stub current_user
